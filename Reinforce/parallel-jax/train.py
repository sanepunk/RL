...
def train(
    ...
    optimizer = return_optimizer(
        model, "adam", learning_rate=learning_rate, gradNorm=1.0
    )
    vmap_run_episode = jax.vmap(
        lambda model, key: run_episode(model, env, env_params, rng=key, steps=500),
        in_axes=(None, 0),
    )
    ...
    def update(model: Agent, optimizer, rng_batch):
        ...
        returns = jax.vmap(calculate_returns, in_axes=(0, 0, None))(
            trajectories.get("reward"), trajectories.get("valid_mask"), gamma
        )
        ...
        std_return = (
            jnp.sqrt(
                jnp.sum(((returns - mean_return) ** 2) * trajectories.get("valid_mask"))
                / mask_sum
            )
            + 1e-8
        )
        ...
        def loss(model: Agent, obs, action):
            ...
            log_softmax_logits = nnx.log_softmax(logits).reshape(
                batch_size, steps_size, -1
            )
            logits_along_action = jnp.take_along_axis(
                log_softmax_logits, action[:, :, None], axis=2
            ).squeeze(2)
            returns_loss = jnp.sum(
                logits_along_action * returns * trajectories.get("valid_mask")
            )
        ...
        model.train()
        loss, grad = nnx.value_and_grad(loss, argnums=0)(
            model, trajectories.get("obs"), trajectories.get("action")
        )
        optimizer.update(model, grad)

        avg_reward = jnp.mean(
            jnp.sum(trajectories.get("reward") * trajectories.get("valid_mask"), axis=1)
        )
